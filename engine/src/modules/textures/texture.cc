...
texture::texture(char* filename, int target, int flags) : img(NULL), flags(flags){
	load(filename, target, flags);
}
...
bool texture::load(char* filename, int target, int flags){
	this->flags = flags;
	this->target = target;
	...
	if (flags &RGB){
		...
		std::cout <<  "texture.cc -> formato de textura invalido" << std::endl;
	}
