                           ...
                           const Option _WeightNormalsByFaceSize )
{
    ...
    if ( bNeedNormals || bComputeTangentSpace )
    {
        ...
        if ( !have_normals )
        {
            ...
            {
                output[ (*want).second ].floatVector_[ n ] = 0;
            }
		...
		for ( unsigned int f = 0; f < theSize; f += 3 )
		{
			...
			if ( _FixTangents == FixTangents )
			{
				...
				for ( int e = 0; e < 3; ++e )
				{
					...
					edge.v0 = (unsigned int)min( (float)indices[ start ], (float)indices[ end ] );
					edge.v1 = (unsigned int)max( (float)indices[ start ], (float)indices[ end ] );
