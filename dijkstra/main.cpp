...
char* name[] = {"HNL","SFO","LAX","ORD","DFW","LGA", "PVD", "MIA"};
...
unsigned int weight[no_vertices][no_vertices] =
  {
...
unsigned int D[no_vertices]; 
bool tight[no_vertices];
unsigned int predecessor[no_vertices];
...
vertex minimal_nontight() {
  ...
  for (j = first_vertex; j <= last_vertex;){
     if (!tight[j]) 
        break;
...
void dijkstra(vertex s) {
  ...
  for (z = first_vertex; z <= last_vertex;) {
...
unsigned int stack[stack_limit];
